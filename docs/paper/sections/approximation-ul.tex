...
\begin{equation}
  \label{eq:tul}
  \boxed{\Tul = \Tub - \frac{\rho_1+\rho_2}{8}\,(\Tub - \Tbot),}
\end{equation}
where $\Tub$ and $\Tbot$ are given by~\eqref{eq:tub} and~\eqref{eq:tbot}.
The approximation lowers the upper bound by a fraction
$\alpha = (\rho_1+\rho_2)/8$ of the bound gap $\Tub - \Tbot$: it sits at $\Tub$
under light load and moves toward $\Tbot$ as utilization rises.
...
Substituting:
\begin{equation}
  \Tul = \frac{3}{2}T_1 - \frac{\rho}{4}\left(\frac{3}{2}T_1 - T_1\right)
       = \frac{3}{2}T_1 - \frac{\rho}{8}T_1
